classdef CommClient < handle
    ...
    properties
        ...
        messageLog = {};    % Stores the last 100 received messages
        lastHeartbeat = NaT;% Last heartbeat timestamp

        defaultTopics       % Struct with topic layout: cmd, status, data, log
        ...
        heartbeatInterval = 0;          % seconds between status updates (user-specified)
    ...
    methods
        ...
        function obj = CommClient(cfg)
            ...
            if isfield(cfg, 'onMessageCallback')
                if isa(cfg.onMessageCallback, 'function_handle')
                    ...
                    error('CommClient:InvalidCallback', 'onMessageCallback must be a function handle.');
                end
            end
            ...
            if hasCustomSubs
                if ~(iscellstr(cfg.subscriptions) || isstring(cfg.subscriptions))
                    error('CommClient:InvalidSubscriptions', 'subscriptions must be a cell array or string array.');
                end
                obj.subscriptions = cfg.subscriptions;
            end
            if hasCustomPubs
                if ~(iscellstr(cfg.publications) || isstring(cfg.publications))
                    error('CommClient:InvalidPublications', 'publications must be a cell array or string array.');
                end
            ...
            if obj.verbose
                fprintf('[CommClient] Initialized for clientID: %s\n', obj.clientID);
                fprintf('[CommClient] Broker: %s:%d\n', obj.brokerAddress, obj.brokerPort);
                fprintf('[CommClient] Subscribed topics: %s\n', strjoin(obj.subscriptions, ', '));
                fprintf('[CommClient] Publication topics: %s\n', strjoin(obj.publications, ', '));
            end
        end
        ...
        function delete(obj)
            ...
            obj.disconnect();
            if obj.verbose
                fprintf('[CommClient] Object deleted and resources released.\n');
            end
        end
        ...
        function connect(obj)
            ...
            try
                if obj.verbose
                    disp('[CommClient] connect() method reached.');
                    fprintf('[CommClient] Attempting to connect to MQTT broker at %s:%d\n', obj.brokerAddress, obj.brokerPort);
                end
                ...
                for i = 1:length(obj.subscriptions)
                    ...
                    if obj.verbose
                        fprintf('[CommClient] Subscribed to topic: %s\n', topic);
                    end
                end
                ...
                if obj.heartbeatInterval > 0
                    ...
                    if obj.verbose
                        fprintf('[CommClient] Heartbeat timer started with interval %.1f sec\n', obj.heartbeatInterval);
                    end
                end

                if obj.verbose && obj.mqttClient.Connected
                    fprintf('[CommClient] Successfully connected and subscribed.\n');
                end

            catch ME
                if obj.verbose
                    fprintf('[CommClient] ERROR during connection: %s\n', ME.message);
                end
                rethrow(ME);
            end
        end
        ...
        function disconnect(obj)
            ...
            if ~isempty(obj.mqttClient) && obj.mqttClient.Connected
                try
                    ...
                    if obj.verbose
                        fprintf('[CommClient] Disconnected from broker and cleaned up.\n');
                    end
                catch ME
                    warning('CommClient:DisconnectWarning', 'Error while disconnecting: %s', ME.message);
                end
            end
        end
        ...
        function commPublish(obj, topic, payload)
            ...
            if isempty(obj.mqttClient) || ~obj.mqttClient.Connected
                error('CommClient:NotConnected', 'MQTT client is not connected. Call connect() first.');
            end

            try
                write(obj.mqttClient, topic, payload);
                if obj.verbose
                    fprintf('[CommClient] Published to topic "%s": %s\n', topic, payload);
                end
            catch ME
                if obj.verbose
                    fprintf('[CommClient] ERROR during publish to topic "%s": %s\n', topic, ME.message);
                end
                rethrow(ME);
            end
        end
        ...
        function sendHeartbeat(obj)
            ...
            if isempty(obj.mqttClient) || ~obj.mqttClient.Connected
                if obj.verbose
                    fprintf('[CommClient] Skipped heartbeat: MQTT client is not connected.\n');
                end
                return;
            end
            ...
            if obj.verbose
                fprintf('[CommClient] Heartbeat sent to %s\n', topic);
            end
        end
        ...
        function commSubscribe(obj, topic)
            ...
            if isempty(obj.mqttClient) || ~obj.mqttClient.Connected
                error('CommClient:NotConnected', 'MQTT client is not connected.');
            end

            if any(strcmp(obj.subscriptions, topic))
                if obj.verbose
                    fprintf('[CommClient] Topic "%s" already subscribed.\n', topic);
                end
                return;
            end
            ...
            if obj.verbose
                fprintf('[CommClient] Successfully subscribed to topic: %s\n', topic);
            end
        end
        ...
        function commUnsubscribe(obj, topic)
            ...
            if ~any(strcmp(obj.subscriptions, topic))
                if obj.verbose
                    fprintf('[CommClient] Topic "%s" is not currently subscribed.\n', topic);
                end
                return;
            end
            ...
            if obj.verbose
                fprintf('[CommClient] Successfully unsubscribed from topic: %s\n', topic);
            end
        end
        ...
        function handleMessage(obj, topic, msg)
            ...
            if obj.verbose
                fprintf('[CommClient] Received message on topic "%s": %s\n', topic, msg);
            end
            ...
            if ~isempty(obj.onMessageCallback)
                try
                    ...
                    warning('CommClient:CallbackError', 'Error in onMessageCallback: %s', callbackErr.message);
                end
            end
        end
        ...
        function topic = getFullTopic(obj, suffix)
            ...
            if ~ischar(suffix) && ~isstring(suffix)
                error('CommClient:InvalidSuffix', 'Suffix must be a string or character array.');
            end

            topic = sprintf('%s/%s', obj.clientID, char(suffix));

            if obj.verbose
                fprintf('[CommClient] getFullTopic generated: %s\n', topic);
            end
        end

        function addToLog(obj, topic, msg)
            % ADDTOLOG - Stores topic-message pair in messageLog with timestamp.
            % Keeps only the last 100 entries (FIFO ring buffer logic).

            if nargin < 3 || isempty(topic) || isempty(msg)
                error('CommClient:InvalidLogEntry', 'Topic and message are required to log an entry.');
            end
            ...
            if numel(obj.messageLog) > 100
                obj.messageLog(1) = [];
            end

            if obj.verbose
                fprintf('[CommClient] Logged message on topic "%s" at %s\n', topic, string(entry.timestamp));
            end
        end
    end
end